...
4. Commit policy: the implementer commits progress on the run's branch in reasonable chunks as standing policy of this workflow, and pushes only where the plan calls for it (for example so CI can verify a gate). The orchestrator creates the run branch, creates the draft PR when one is wanted, pushes the run branch, and keeps the PR description current; those need no separate approval. Never push anywhere beyond the run branch and never mark the PR ready for review without explicit user approval.
...
Stop the wakeup loop. Run the gates one final time yourself and spot-check the diff; do not take the last agent's word for it. Commit per the kickoff policy, push the run branch, and update the PR description to match what actually shipped: pushing the run branch and editing its PR are the orchestrator's job, not something to leave for the user. Confirm nothing is left running. Then report to the user: what shipped, measured results, validation history (rounds, majors and minors, recorded declines), the commits made, and open items.
